...
interface EditTripModalDriverProps {
  isOpen: boolean
  onClose: () => void
  onSuccess: () => void
  tripId: string
...
export function EditTripModalDriver({ isOpen, onClose, onSuccess, tripId, driverId }: EditTripModalDriverProps) {
  ...
  const fetchTrip = async () => {
    try {
      ...
      if (response.ok) {
        ...
        const dateStr = tripData.date ? new Date(tripData.date).toISOString().split('T')[0] : ''
        setFormData({
          materialId: tripData.materialId || '',
          projectId: tripData.projectId || '',
          date: dateStr,
          vehicleId: tripData.vehicleId || '',
          incomingReceiptNumber: tripData.incomingReceiptNumber || '',
          outcomingReceiptNumber: tripData.outcomingReceiptNumber || '',
          quantity: tripData.quantity?.toString() || '',
          measure: tripData.measure || '',
          observation: tripData.observation || ''
        })

        // Establecer proyecto y material seleccionados
        if (tripData.project) {
          setSelectedProject(tripData.project)
        }
        if (tripData.material) {
          setSelectedMaterial(tripData.material)
        }
      }
    } catch (error) {
      console.error('Error fetching trip:', error)
      setErrors({ submit: 'Error al cargar el viaje' })
    }
  }
    ...
    try {
      const tripResponse = await fetch(`/api/trips/${tripId}`, {
        ...
        body: JSON.stringify({
          ...
          date: formData.date,
          driverId: driverId, // Mantener el mismo conductor
          ...
          // No modificar los precios, se mantienen los existentes
  ...
  // Si el viaje está aprobado, mostrar mensaje de error
  if (trip?.isApproved) {
    return (
        ...
        <Card className="w-full max-w-md" onClick={(e) => e.stopPropagation()}>
          ...
          <CardContent>
            <p className="text-gray-700 mb-4">
              Este viaje ya está aprobado y no puede ser editado.
            </p>
