...
export function MaterialAutocomplete({
  ...
  const handleRemoveMaterial = (materialId: string) => {
    onMaterialsChange(selectedMaterials.filter(m => m.id !== materialId))
  }
    ...
    <div className="space-y-2 relative">
        ...
        <div className="space-y-2">
          ...
          <div className="space-y-2">
            {selectedMaterials.map((material) => (
              <div key={material.id} className="flex items-center justify-between p-2 bg-gray-50 rounded-lg border">
      ...
      {selectedMaterials.length === 0 && (
        <div className="p-4 border-2 border-dashed border-gray-300 rounded-lg text-center">
          ...
          <p className="text-sm text-muted-foreground">
            {multiple ? 'No hay materiales seleccionados' : 'Ningún material seleccionado'}
          </p>
        </div>
      )}
    </div>
