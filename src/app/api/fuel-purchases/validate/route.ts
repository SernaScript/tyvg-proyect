...
// POST - Validar archivo Excel con compras de combustible sin guardar en BD
export async function POST(request: NextRequest) {
  try {
    ...
    for (let i = 0; i < data.length; i++) {
      ...
      try {
        ...
        if (fecha && typeof fecha === 'object') {
          if (fecha.constructor === Date) {
            ...
            if (!isNaN(fecha.getTime())) {
              ...
              results.invalidRecords.push({ rowNumber, row, error: 'Fecha inválida' })
              continue
            }
            ...
            try {
              ...
              results.invalidRecords.push({ rowNumber, row, error: 'Error al procesar fecha' })
              continue
            }
          } else {
        ...
        if (!fecha || !vehiculo || !cantidad || !total || !proveedor) {
          ...
          results.invalidRecords.push({ rowNumber, row, error: 'Faltan campos obligatorios' })
          continue
        }
        ...
        if (!isNaN(Number(fechaStr)) && Number(fechaStr) > 0) {
          ...
          if (serialNumber >= 1 && serialNumber <= 2958465) { // Rango válido para fechas
            try {
              ...
              results.invalidRecords.push({ rowNumber, row, error: 'Error al convertir fecha' })
              continue
            }
          } else {
            results.errors++
            results.errorDetails.push(`Fila ${rowNumber}: Serial de Excel fuera de rango: ${fecha}`)
            results.invalidRecords.push({ rowNumber, row, error: 'Fecha fuera de rango' })
            continue
          }
        }
        // Verificar si es formato dd/mm/aaaa
          ...
          if (parts.length === 3) {
            ...
            results.invalidRecords.push({ rowNumber, row, error: 'Formato de fecha inválido' })
            continue
          }
        } else {
        ...
        if (!vehicleId) {
          ...
          results.invalidRecords.push({ rowNumber, row, error: 'Vehículo no encontrado' })
          continue
        }
        ...
        if (isNaN(cantidadNum) || cantidadNum <= 0) {
          ...
          results.invalidRecords.push({ rowNumber, row, error: 'Cantidad inválida' })
          continue
        }
        ...
        if (isNaN(totalNum) || totalNum <= 0) {
          ...
          results.invalidRecords.push({ rowNumber, row, error: 'Total inválido' })
          continue
        }
        ...
        results.invalidRecords.push({ rowNumber, row, error: 'Error interno' })
      }
    }
    ...
    console.error('Error validating fuel purchases file:', error)
    return NextResponse.json(
      { error: 'Error al validar el archivo de compras de combustible' },
      { status: 500 }
    )
  }
}
